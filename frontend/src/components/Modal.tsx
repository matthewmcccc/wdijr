...
const Modal = ({ showModal, setShowModal, setBlur, setBookId, mode }: { showModal: boolean, setShowModal: React.Dispatch<React.SetStateAction<boolean>>, setBlur: React.Dispatch<React.SetStateAction<boolean>>, setBookId: React.Dispatch<React.SetStateAction<string>>, mode: "upload" | "preprocessed" }) => {
    ...
    if (mode == "preprocessed") {
        return (
            <div className="fixed w-1/3 rounded-lg h-1/3 mx-auto my-auto inset-0 bg-white border border-black flex flex-col z-50 animate-fade-in">
                <button className="absolute cursor-pointer top-2 right-2 text-gray-500 hover:text-gray-700 text-3xl leading-none mt-2" onClick={() => { setShowModal(false); setBlur(false); setSelectedBook(null); }}>
                    &times;
                </button>
